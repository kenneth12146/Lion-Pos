...
function filtrarPorCategoria(categoria) {
  ...
  // marcar boton correspondiente
  const btn = Array.from(document.querySelectorAll('.cat-btn')).find(b => b.dataset.categoria === categoria) || document.querySelector('.cat-btn');
...
function recuperarVentaPendiente(index) {
  ...
  // CORRECCIÓN: clonar correctamente
...
document.getElementById('btnConfirmarVenta').addEventListener('click', () => {
  ...
  if (totalAPagarEl) totalAPagarEl.textContent = formatMoney(total);
  ...
  if (cambioTextoEl) cambioTextoEl.textContent = '$ 0';
  
  const modalPago = document.getElementById('modalPago');
  if (modalPago) modalPago.classList.add('show');
  if (montoRecibidoEl) montoRecibidoEl.focus();
});
...
document.querySelectorAll('.metodo-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (efectivoSection) efectivoSection.style.display = metodoPagoSeleccionado === 'efectivo' ? 'block' : 'none';
  });
});
...
if (montoRecibidoInput) {
  montoRecibidoInput.addEventListener('input', (e) => {
    ...
    const cambioTextoEl = document.getElementById('cambioTexto');
    if (cambioTextoEl) {
      cambioTextoEl.textContent = formatMoney(Math.max(0, cambio));
      cambioTextoEl.style.color = cambio >= 0 ? '#27ae60' : '#e74c3c';
    }
  });
}
...
document.getElementById('btnFinalizarVenta').addEventListener('click', async () => {
  ...
  try {
    ...
    // CORRECCIÓN: actualizar stock usando spread correcto
    ...
    mostrarMensaje('❌ Error: ' + (err?.message || String(err)), 'error');
  }
});
...
async function procesarDevolucion() {
  ...
  try {
    ...
    // REGISTRAR DEVOLUCIÓN EN EL STORAGE
    await Storage.registrarDevolucion(devolucion);
    console.log('✅ Devolución registrada en Storage');
    
    // DEVOLVER STOCK A LOS PRODUCTOS
    for (const item of itemsDevolver) {
      const producto = productos.find(p => p.codigo === item.codigo || p.id === item.id);
      if (producto) {
        const nuevoStock = (producto.cantidad || 0) + (item.cantidad || 0);
        await Storage.editarProducto(producto.id, { 
          ...producto,
          cantidad: nuevoStock
        });
...
document.getElementById('formSoporte').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    mostrarMensaje('✅ Mensaje enviado. Nos contactaremos pronto.');
...
// ========================================================================
// (No he modificado este bloque salvo mantener su funcionalidad)
(function () {
  ...
  const POSCliente = {
    ...
    // Hooks y funciones de ticket (mantener como estaban)
    async onVentaGuardada(ventaGuardada) {
      console.log('🎯 Post-venta: procesando cliente y ticket', ventaGuardada);
      // Puedes añadir integraciones post-venta aquí si lo deseas.
    },

    imprimirTicket() {
      // lógica de impresión térmica si aplica
    },
  ...
  // Inicializar POSCliente si es necesario
  if (document.readyState === 'complete') {
    POSCliente.init().catch(e => console.error(e));
  } else {
    window.addEventListener('load', () => POSCliente.init().catch(e => console.error(e)));
  }
})();
